    ...
    for i in range(len(turn_list)):
        ...
        if i<len(turn_list)-1:
            ...
            if i>0:
                ...
                pre_time = (15*60*1e3 - np.abs(loc_pos[loc] - loc_pos[pre_loc])/11.7)*0.5
            else:
                pre_time = 0
            post_loc = turn_list[i+1]
            post_time = (15*60*1e3 - np.abs(loc_pos[loc] - loc_pos[post_loc])/11.7)*0.5
            rest_time = pre_time + 15*60*1e3 + post_time
